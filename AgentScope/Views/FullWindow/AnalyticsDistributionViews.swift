...
struct CostByWorkspaceView: View {
    ...
    var topWorkspaces: [WorkspaceCost] {
        Array(workspaceCosts.prefix(8))
    }

    var body: some View {
        CardView {
            VStack(alignment: .leading, spacing: 12) {
                Text("Cost by Workspace")
                    .font(.system(size: 13, weight: .medium))

                VStack(spacing: 8) {
                    ForEach(Array(topWorkspaces.enumerated()), id: \.element.id) { index, workspace in
                        VStack(alignment: .leading, spacing: 4) {
                            HStack {
                                Text(workspace.workspaceName)
                                    ...
                                    .lineLimit(1)
                                Spacer()
                                Text(formatCost(workspace.totalCost))
                                    .font(Typography.code)
                                    .foregroundStyle(.secondary)
                                if totalCost > 0 {
                                    Text("\(Int((workspace.totalCost / totalCost) * 100))%")
                                        .font(.system(size: 11))
                            ...
                            GeometryReader { geo in
                                RoundedRectangle(cornerRadius: 2)
                                    ...
                                    .frame(width: max(4, geo.size.width * (totalCost > 0 ? workspace.totalCost / totalCost : 0)))
                            }
                            .frame(height: 4)
                        }
                    }
