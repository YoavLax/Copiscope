...
struct TimelineSidebarContent: View {
    ...
    private var filteredEntries: [HistoryEntry] {
        ...
        return entries.filter { entry in
            ...
            (entry.workspace?.localizedCaseInsensitiveContains(filterText) ?? false) ||
        ...
        Button(action: onSelect) {
            HStack(alignment: .top, spacing: 6) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    if let label = workspaceLabel(entry.workspace) {
                        Text(label)
                            .font(Typography.caption)
...
struct TimelineMainPanelView: View {
        ...
        Group {
            if isLoading {
                ...
                EmptyStateView(
                    ...
                    message: "History entries from your Copilot sessions will appear here."
                )
            } else {
                timelineContent
            }
    ...
    private func timelineRow(entry: HistoryEntry, previousEntry: HistoryEntry?, gap: TimeGap) -> some View {
        let workspaceColor = colorForWorkspace(entry.workspace)
        ...
        let showBadge = shouldShowWorkspaceBadge(entry, previousEntry: previousEntry, gap: gap)
        ...
        HStack(alignment: .top, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: Spacing.xs) {
                ...
                if showSession, let info = sessionInfo(for: entry.sessionId) {
                    HStack {
                        ...
                        if onNavigateToSession != nil {
                            Button {
                                onNavigateToSession?(info.workspaceId, info.sessionId)
                            } label: {
                                HStack(spacing: 3) {
                                    Text(info.title)
                ...
                if showBadge, let label = workspaceLabel(entry.workspace) {
                    Text(label)
                        .font(Typography.caption)
                        .foregroundStyle(workspaceColor)
                        .padding(.horizontal, 5)
                        .padding(.vertical, 1)
                        .background(workspaceColor.opacity(0.12))
                        .clipShape(Capsule())
                }
            }
    ...
    private func shouldShowWorkspaceBadge(_ entry: HistoryEntry, previousEntry: HistoryEntry?, gap: TimeGap) -> Bool {
        guard let prev = previousEntry else { return true }
        if gap == .large { return true }
        return entry.workspace != prev.workspace
    }
    ...
    private func sessionInfo(for sessionId: String?) -> (title: String, workspaceId: String, sessionId: String)? {
        guard let sessionId else { return nil }
        for (workspaceId, sessions) in store.sessionsByWorkspace {
            if let match = sessions.first(where: { $0.id == sessionId }) {
                return (match.title, workspaceId, sessionId)
            }
        }
        return nil
    }
    ...
    private func colorForWorkspace(_ path: String?) -> Color {
        guard let path else { return Self.workspaceColors[0] }
        let hash = abs(path.hashValue)
        let index = hash % Self.workspaceColors.count
        return Self.workspaceColors[index]
    }
}
