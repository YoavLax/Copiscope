...
struct AnalyticsDetailView: View {
    ...
    var selectedWorkspaceName: String? {
        ...
        return store.workspaces.first(where: { $0.id == id })?.name
    }
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 20) {
                        ...
                        HStack(alignment: .top, spacing: 16) {
                            CostByWorkspaceView(workspaceCosts: data.workspaceCosts, totalCost: data.totalCost)
...
struct AnalyticsHeaderView: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            if let name = selectedWorkspaceName {
                HStack(spacing: 4) {
                    Text(name)
