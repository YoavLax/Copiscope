...
struct MemorySidebarContent: View {
    ...
    private var filteredWorkspaces: [Workspace] {
        ...
        return workspaces.filter { $0.name.localizedCaseInsensitiveContains(filterText) }
    }
...
struct MemoryWorkspaceRow: View {
    let name: String
    let icon: String
    let fileCount: Int?
    let isSelected: Bool
    ...
    var body: some View {
        if memoryFiles.isEmpty {
            EmptyStateView(
                ...
                message: "Choose Global or a workspace from the sidebar to view memory files."
            )
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if let content = file.content {
                ...
                EmptyStateView(
                    ...
                    message: "This memory file doesn't exist yet. It will be created when Copilot writes memory for this scope."
                )
            }
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
