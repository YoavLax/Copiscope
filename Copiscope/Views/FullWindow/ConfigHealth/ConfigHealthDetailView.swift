...
struct HealthResultDetailView: View {
    ...
    private var sessionIds: (workspaceId: String, sessionId: String)? {
        guard isSessionResult else { return nil }
        let parts = result.filePath.split(separator: "/")
        guard parts.count >= 3 else { return nil }
        return (workspaceId: String(parts[1]), sessionId: String(parts[2]))
    }
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        if isSessionResult, let ids = sessionIds, let onNavigateToSession {
                            Button {
                                onNavigateToSession(ids.workspaceId, ids.sessionId)
                            } label: {
                                HStack(spacing: 4) {
